...
internal static class PerformanceTimers
{
    ...
    /// </summary>
    /// <returns>Number of performance timer counts since boot.
    /// 
    /// Number of performance timer counts per second can be found in <see cref="PerformanceCountsPerSecond"> property.</see>"/></returns>
    public static long GetPerformanceCounterValue() => Stopwatch.GetTimestamp();
    ...
    {
        perfcount += GetPerformanceCounterValue();

        while (GetPerformanceCounterValue() < perfcount)
        {
        }
    }
    ...
    {
        PerformanceCountsPerSecond = Stopwatch.Frequency;
